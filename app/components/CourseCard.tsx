...
export default function CourseCard({ image, title, description, prompts, category }: CourseCardProps) {
  ...
  const courseCardPreview = (
    <div className="relative rounded-xl sm:rounded-2xl shadow-lg overflow-hidden transition-transform hover:scale-[1.02] h-full w-full cursor-pointer">
      ...
      <div className="absolute bottom-0 left-0 right-0 p-3 xs:p-4 sm:p-5 md:p-6 lg:p-7">
        <h3 className="text-base xs:text-lg sm:text-xl md:text-2xl lg:text-3xl font-bold text-white mb-1 xs:mb-1.5 sm:mb-2">
          {title}
        </h3>
        <p className="text-xs xs:text-sm sm:text-base md:text-lg text-white/90 line-clamp-2">
          {description}
        </p>
      </div>
    </div>
