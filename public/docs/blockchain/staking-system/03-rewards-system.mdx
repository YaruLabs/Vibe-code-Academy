# 💰 Privacy-Preserving Rewards Distribution
...
## 2. Understanding Encrypted Arithmetic 🔢
...
<div style={{
  ...
  <code>rewards = (stakedAmount × timeStaked × APY) / (365 days × 100)</code>
</div>

### All computed in MPC:
- `stakedAmount` → **ctUint64** (encrypted)
- `timeStaked` → **uint256** (public, but result stays encrypted)
- `APY` → **uint256** (public parameter)
- `rewards` → **gtUint64** (encrypted result)
...
## 4. View Pending Rewards (Encrypted) 👀
...
## 5. Emergency Withdraw & Admin Functions 🚨
...
<pre>{`contract PrivateStaking is MpcCore, AccountOnboard, Pausable {
    ...
    // Core Staking
    ✅ stake(itUint64 amount)
    ✅ unstake(itUint64 amount)
    ✅ getStakedBalance() → gtUint64

    // Rewards System
    ✅ calculateRewards(address) → gtUint64 [internal]
    ✅ claimRewards()
    ✅ getPendingRewards() → gtUint64

    // Safety & Admin
    ✅ emergencyWithdraw()
    ✅ setAPY(uint256 newAPY) [onlyOwner]
...
- ✅ Encrypted reward claiming
- ✅ Pending rewards viewer
